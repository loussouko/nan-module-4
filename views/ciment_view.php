...
<div class="container-fluid" style="margin-top: 15px;">
    <div class="row">
        ...
        <div class="col-lg-11 col-12">
            <div class="row">
        <?php $ciments = Product::getProductByCategory(1); foreach ($ciments as $ciment): ?>
        <div class="col-lg-2 mx-4 my-2">
            <div class="card" style="width: 16rem;" >
                <img src="<?= $ciment['imageProduit'] ?>" alt="" class="card-img-top">
                <div class="card-body">
                    <h5><?= $ciment['nomProduit'] ?></h5>
                    <p class="card-text text-danger h5 text-center"><?= $ciment['prixProduit'] ?> FCFA</p>
                    <a href="detailproduct-<?= $ciment['idProduit']?>" class="btn btn-primary"><i class="fa fa-list"></i>Details</a>
                    <?php if($ciment['stockProduit'] != 'epuise'): ?>
                    <a onclick="confirms(<?= $ciment['idProduit'] ?>,'<?= $uri ?>','<?= $urs ?>',<?= $url ?>)" class="btn btn-warning"><i class="fa fa-shopping-cart text-light"></i>Ajouter</a>
                    <?php else: ?>
